#' Update vincentverse packages
#'
#' This will check to see if all vincentverse packages (and optionally, their
#' dependencies) are up-to-date, and will install after an interactive
#' confirmation.
#'
#' @inheritParams vincentverse_deps
#' @export
#' @examples
#' \dontrun{
#' vincentverse_update()
#' }
vincentverse_update <- function(recursive = FALSE, repos = getOption("repos")) {
  deps <- vincentverse_deps(recursive, repos)
  behind <- dplyr::filter(deps, behind)

  if (nrow(behind) == 0) {
    cli::cat_line("All vincentverse packages up-to-date")
    return(invisible())
  }
...
#' Get a situation report on the vincentverse
#'
#' This function gives a quick overview of the versions of R and RStudio as
#' well as all vincentverse packages. It's primarily designed to help you get
#' a quick idea of what's going on when you're helping someone else debug
#' a problem.
#'
#' @export
vincentverse_sitrep <- function() {
...
#' List all vincentverse dependencies
#'
...
#' @export
vincentverse_deps <- function(recursive = FALSE, repos = getOption("repos")) {
  pkgs <- utils::available.packages(repos = repos)
  deps <- tools::package_dependencies("vincentverse", pkgs, recursive = recursive)
