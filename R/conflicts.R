#' Conflicts between the vincentverse and other packages
#'
#' This function lists all the conflicts between packages in the vincentverse
...
#'   with these packages.
#' @examples
#' vincentverse_conflicts()
vincentverse_conflicts <- function(only = NULL) {
  ...
  tidy_names <- paste0("package:", vincentverse_packages())
  conflicts <- purrr::keep(conflicts, function(pkg) any(pkg %in% tidy_names))

  conflict_funs <- purrr::imap(conflicts, confirm_conflict)
  conflict_funs <- purrr::compact(conflict_funs)

  class(conflict_funs) <- "vincentverse_conflicts"
  conflict_funs
}

vincentverse_conflict_message <- function(x) {
  header <- cli::rule(
...
#' @export
print.vincentverse_conflicts <- function(x, ..., startup = FALSE) {
  cli::cat_line(vincentverse_conflict_message(x))
  invisible(x)
}
